...
const BodySchema = z.object({
  slug: z.string().min(1).max(80).regex(/^[a-z0-9-]+$/i, "英数字とハイフンのみ"),
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase
    .from("topics")
    .insert({ slug: body.slug, label: body.label, category: body.category ?? null })
    .select("slug,label,category,created_at")
    .single();
