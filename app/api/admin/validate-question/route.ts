...
export async function POST(req: NextRequest) {
  ...
  function japaneseRatio(s: string) {
    ...
    for (const ch of txt) {
      ...
      if ((code >= 0x3040 && code <= 0x30ff) || (code >= 0x4e00 && code <= 0x9faf) || (code >= 0x3400 && code <= 0x4dbf)) ja++;
    }
    return ja / txt.length;
  }
  function hasProhibitedPhrases(s: string) {
    const t = (s || '').toLowerCase();
    return t.includes('以上すべて') || t.includes('上記すべて') || t.includes('どれでもない') || t.includes('none of the above') || t.includes('all of the above');
  }
  ...
  for (const q of items) {
    try {
      ...
      if (!jaPrompt || !uniqueChoices) {
        results.push({ id: q.id ?? null, pass: false, reason: !jaPrompt ? '日本語として不自然' : '選択肢に重複/不適切語句' });
        continue;
      }
